...
export class Snapflow {
  ...
  private validateCreateParams(params: any): void {
    ...
    if (
      ...
      (!Number.isInteger(autoArchiveInterval) || autoArchiveInterval < 0)
    ) {
  ...
  private async createSandboxData(params: any, timeout: number) {
    ...
    const response = await this.sandboxApi.createSandbox(createParams, undefined, {
  ...
  private async handleBuildLogs(
    ...
    await processStreamingResponse(
      ...
      async () => {
        sandboxInstance = (await this.sandboxApi.getSandbox(sandboxInstance.id)).data;
        return (
          sandboxInstance.state !== undefined && terminalStates.includes(sandboxInstance.state)
        );
      }
    );

  ...
  return (
    <div className="flex flex-1 flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className={cn("text-[11px]", pct > 90 ? "text-red-400" : "text-text-secondary")}>
...
export function UsageAndCost() {
  ...
  const fetchUsage = useCallback(async () => {
    ...
    try {
      const response = await apiClient.organizationsApi.getOrganizationUsageOverview(
    ...
    <div className="rounded-lg border border-border">
      <div className="flex items-center justify-between border-border border-b px-5 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-[13px] text-text-body">Resource Usage</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-center px-5 py-8">
          <span className="text-[13px] text-text-secondary">Unable to load usage data</span>
        </div>
      )}
    </div>
  );

...
const LIMIT_ROWS: { label: string; getValue: (t: BillingTier) => string }[] = [
  { label: "Concurrent Sandboxes", getValue: (t) => fmtLimit(t.maxConcurrentSandboxes) },
  { label: "Max Lifetime", getValue: (t) => fmtLife(t.maxSandboxLifetimeSeconds) },
  { label: "Max vCPUs", getValue: (t) => fmtLimit(t.maxCpuPerSandbox) },
  { label: "Max Memory", getValue: (t) => fmtMem(t.maxMemoryPerSandbox) },
  {
    label: "Storage",
    getValue: (t) => (t.maxStorageTotal === -1 ? "Custom" : `${t.maxStorageTotal} GB`),
  },
...
export function TierComparisonTable({ tiers, currentTier }: TierComparisonTableProps) {
  return (
    <div className="mb-8 overflow-hidden rounded-lg border border-border">
      <div className="border-border border-b px-5 py-3">
        <div className="flex items-center gap-2">
          <Shield className="h-3.5 w-3.5 text-text-icon" />
          <span className="font-medium text-[13px] text-text-body">All Tiers</span>
        </div>
        <p className="mt-1 text-[11px] text-text-muted">
          Your tier is determined by your wallet balance. Add funds to unlock higher limits.
        </p>
      </div>
                ...
                return (
                  <th
                    ...
                    className={cn(
                      ...
                      isCurrent ? "bg-surface-active text-text-primary" : "text-text-muted"
                    )}
                  >

...
export function TransactionTable() {
	...
	const fetchTransactions = useCallback(async () => {
		...
		setLoading(true);
		try {
			const response = await apiClient.billingApi.listWalletTransactions(
				...
				<table className="w-full text-[12px]">
					<thead>
						<tr className="border-border border-b">
							<th className="px-5 py-2 text-left font-medium text-text-muted">
								Description
							</th>
							<th className="px-5 py-2 text-right font-medium text-text-muted">
								Amount
							</th>
							<th className="px-5 py-2 text-right font-medium text-text-muted">
								Balance

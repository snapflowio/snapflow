...
import {
	type CreateInvitationRoleEnum,
	type OrganizationInvitation,
	OrganizationInvitationStatusEnum,
	type OrganizationUser,
	OrganizationUserRoleEnum,
	type UpdateInvitationRoleEnum,
	type UpdateMemberRoleRoleEnum,
...
import { CancelOrganizationInvitationModal } from '@/features/members/components/cancel-organization-invitation-modal';
import { CreateOrganizationInvitationModal } from '@/features/members/components/create-organization-invitation-modal';
import { RemoveOrganizationMemberModal } from '@/features/members/components/remove-organization-member-modal';
import { UpdateAssignedOrganizationRolesModal } from '@/features/members/components/update-assigned-organization-roles-modal';
import { UpdateOrganizationInvitationModal } from '@/features/members/components/update-organization-invitation-modal';
import { UpdateOrganizationMemberRolesModal } from '@/features/members/components/update-organization-member-roles-modal';
...
export default function MembersPage() {
	...
	const [loadingMemberAction, setLoadingMemberAction] = useState<
		Record<string, boolean>
	>({});
	const [loadingInvitationAction, setLoadingInvitationAction] = useState<
		Record<string, boolean>
	>({});
	...
	const [invitationToEdit, setInvitationToEdit] =
		useState<OrganizationInvitation | null>(null);
	...
	const handleUpdateMemberRole = useCallback(
		async (
			...
			try {
				await organizationsApi.updateRoleForOrganizationMember(
					selectedOrganization.id,
					userId,
					{
						role,
		...
		[selectedOrganization, refreshOrganizationMembers]
	);
	...
	const handleUpdateAssignedOrganizationRoles = useCallback(
		async (userId: string, roleIds: string[]): Promise<boolean> => {
			...
			setLoadingMemberAction((prev) => ({ ...prev, [userId]: true }));
			try {
				await organizationsApi.updateAssignedOrganizationRoles(
					selectedOrganization.id,
					userId,
					{
						roleIds,
	...
	const handleRemoveMember = useCallback(
		async (userId: string): Promise<boolean> => {
			...
			setLoadingMemberAction((prev) => ({ ...prev, [userId]: true }));
			try {
				await organizationsApi.deleteOrganizationMember(
					selectedOrganization.id,
					userId
				);
	...
	const handleCreateInvitation = useCallback(
		...
		[selectedOrganization, fetchInvitations]
	);
	...
	const handleUpdateInvitation = useCallback(
		async (
			...
			try {
				await organizationsApi.updateOrganizationInvitation(
					invitationId,
					selectedOrganization.id,
					{
						role,
	...
	const handleCancelInvitation = useCallback(
		async (invitationId: string): Promise<boolean> => {
			...
			setLoadingInvitationAction((prev) => ({ ...prev, [invitationId]: true }));
			try {
				await organizationsApi.cancelOrganizationInvitation(
					invitationId,
					selectedOrganization.id
				);
		...
		<div className="flex h-full flex-col gap-0 overflow-y-auto">
			...
			{memberToChangeAssignedRoles && (
				<UpdateAssignedOrganizationRolesModal
					...
					onOpenChange={(open) => {
						if (!open) {
							setMemberToChangeAssignedRoles(null);
						}
					}}
					...
					onUpdateAssignedRoles={(roleIds: string[]) =>
						handleUpdateAssignedOrganizationRoles(
							memberToChangeAssignedRoles.userId,
							roleIds
						)
					}
					loading={
						loadingMemberAction[memberToChangeAssignedRoles.userId] ?? false
					}
				/>
			)}
			...
			{invitationToCancel && (
				<CancelOrganizationInvitationModal
					...
					onCancelInvitation={() =>
						handleCancelInvitation(invitationToCancel.id)
					}

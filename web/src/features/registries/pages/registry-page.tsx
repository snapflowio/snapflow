...
import { type Registry, RegistryRegistryTypeEnum } from '@snapflow/api-client';
import { Container, Edit, MoreVertical, Trash2 } from 'lucide-react';
import { useCallback, useEffect, useMemo, useState } from 'react';
import { apiClient } from '@/api/api-client';
import { CreateRegistryModal } from '@/features/registries/components/create-registry-modal';
import { EditRegistryModal } from '@/features/registries/components/edit-registry-modal';
...
> = {
	...
	[RegistryRegistryTypeEnum.ORGANIZATION]: {
		label: 'Organization',
		variant: 'purple',
	},
...
export default function RegistryPage() {
	...
	const [loadingRegistries, setLoadingRegistries] = useState<
	...
	const fetchRegistries = useCallback(
		async (showTableLoadingState = true) => {
			...
			try {
				const response = await registryApi.listRegistries(
	...
	useEffect(() => {
		const handleRegistryCreatedEvent = (registry: Registry) => {
			setRegistries((prev) => {
				if (prev.some((r) => r.id === registry.id)) {
					return prev;
				}
		...
		const handleRegistryUpdatedEvent = (data: { registry: Registry }) => {
			setRegistries((prev) =>
				prev.map((r) => (r.id === data.registry.id ? data.registry : r))
			);
		};
	...
	const handleDeleteRegistry = async () => {
		if (!registryToDelete || !selectedOrganization) {
			return;
		}
		setLoadingRegistries((prev) => ({ ...prev, [registryToDelete.id]: true }));
		try {
			await registryApi.deleteRegistry(
				registryToDelete.id,
				selectedOrganization?.id
			);
			...
			setLoadingRegistries((prev) => ({
				...prev,
				[registryToDelete.id]: false,
			}));
	...
	const handleEditRegistry = async (data: {
		...
		if (!registryToEdit || !selectedOrganization) {
			return;
		}
				...
				<ModalContent size="sm">
					...
					<ModalBody>
						<p className="font-season text-[14px] text-text-secondary">
							...
							{registryToDelete?.name}"? This action cannot be undone.
						</p>
					</ModalBody>
					<ModalFooter>
						<button
						...
						>
							{loadingRegistries[registryToDelete?.id || '']
								? 'Deleting...'
								: 'Delete'}
						</button>
					</ModalFooter>

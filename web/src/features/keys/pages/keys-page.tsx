...
import { CreateApiKeyModal } from '@/features/keys/components/create-api-key-modal';
...
function ExpiresCell({ expiresAt }: { expiresAt: Date | null }) {
	if (!expiresAt) {
		return (
			<span className="font-season text-[13px] text-text-secondary">Never</span>
		);
	}
	const now = new Date();
			...
			return (
				<DropdownMenu>
					...
					<DropdownMenuContent align="end" sideOffset={4}>
						<DropdownMenuItem
							onClick={() => setKeyToRevoke(id)}
							disabled={loadingKeys[id]}
						>
			...
			>
				<ModalContent size="sm">
					...
					<ModalFooter>
						...
						>
							{keyToRevoke && loadingKeys[keyToRevoke]
								? 'Revoking...'
								: 'Revoke'}
						</button>
					</ModalFooter>

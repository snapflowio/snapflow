...
import { CreateSandboxModal } from '../components/create-sandbox-modal';
...
export default function SandboxesPage() {
	...
	const [loadingSandboxes, setLoadingSandboxes] = useState<
	...
	const fetchSandboxes = useCallback(
		async (showLoading = true) => {
			...
			try {
				const data = (await sandboxApi.listSandboxes(selectedOrganization.id))
					.data;
				setSandboxes(data);
			} catch (error) {
	...
	useEffect(() => {
		...
		const onDesiredStateUpdated = (d: {
			...
			if (
				...
				(d.sandbox.state === SandboxState.ERROR ||
					d.sandbox.state === SandboxState.BUILD_FAILED)
			) {
				setSandboxes((prev) => prev.filter((s) => s.id !== d.sandbox.id));
			}
		};
	...
	const handleStart = useCallback(
		async (id: string) => {
			...
			setSandboxes((p) =>
				p.map((s) => (s.id === id ? { ...s, state: SandboxState.STARTING } : s))
			);
			try {
				...
				setSandboxes((p) =>
					p.map((s) => (s.id === id ? { ...s, state: prev } : s))
				);
			} finally {
				setLoadingSandboxes((p) => ({ ...p, [id]: false }));
			}
		},
		[sandboxes, selectedOrganization]
	);
	...
	const handleStop = useCallback(
		async (id: string) => {
			...
			const prev = sandboxes.find((s) => s.id === id)?.state;
			setSandboxes((p) =>
				p.map((s) => (s.id === id ? { ...s, state: SandboxState.STOPPING } : s))
			);
			try {
	...
	const handleDelete = async (id: string) => {
		...
		const prev = sandboxes.find((s) => s.id === id)?.state;
		setSandboxes((p) =>
			p.map((s) => (s.id === id ? { ...s, state: SandboxState.DESTROYING } : s))
		);
		try {
	...
	const handleArchive = useCallback(
		async (id: string) => {
			...
			setSandboxes((p) =>
				p.map((s) =>
					s.id === id ? { ...s, state: SandboxState.ARCHIVING } : s
				)
			);
			try {
	...
	const handleVnc = useCallback(
		async (sandbox: Sandbox) => {
			...
			try {
				...
				const toolboxConfig = new ToolboxConfiguration({
					basePath: sandbox.toolboxProxyUrl,
				});
				const computerApi = new ComputerUseApi(
					toolboxConfig,
					'',
				...
				const preview = (
					await sandboxApi.getPortPreviewUrl(
						sandbox.id,
						6080,
	...
	const filtered = useMemo(() => {
		...
		return sandboxes.filter(
			(s) =>
				...
				(s.labels &&
					Object.entries(s.labels).some(([k, v]) =>
			...
			>
				<ModalContent size="sm">
					...
					<ModalFooter>
						...
						>
							{sandboxToDelete && loadingSandboxes[sandboxToDelete]
								? 'Deleting...'
								: 'Delete'}
						</button>
					</ModalFooter>

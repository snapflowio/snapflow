...
import { Image, ImageState, PaginatedImages } from "@snapflow/api-client";
import { ImageIcon, MoreVertical, Power, Trash2 } from "lucide-react";
import { CreateImageModal } from "@/components/modals/create-image-modal";
...
export default function ImagesPage() {
  ...
  const fetchImages = useCallback(
    async (showTableLoadingState = true) => {
      ...
      try {
        const response = (await imageApi.listImages(selectedOrganization.id, 1, MAX_PAGE_SIZE))
          .data;
  ...
  useEffect(() => {
    const handleImageCreatedEvent = (image: Image) => {
      setImagesData((prev) => {
        if (prev.items.some((i) => i.id === image.id)) return prev;
        const insertIndex =
        ...
        return { ...prev, items: newImages, total: newTotal, totalPages: prev.totalPages };
      });
    };
    ...
    const handleImageRemovedEvent = (imageId: string) => {
      setImagesData((prev) => {
        ...
        return { ...prev, items: newItems, total: newTotal, totalPages: prev.totalPages };
      });
    };
  ...
  const handleActivate = async (image: Image) => {
    setLoadingImages((prev) => ({ ...prev, [image.id]: true }));
    setImagesData((prev) => ({
      ...prev,
  ...
  const filtered = useMemo(() => {
    if (!searchQuery) return imagesData.items;
  ...
  const rows: ResourceRow[] = useMemo(
    () =>
      filtered.map((img) => ({
        ...
        sortValues: {
          created: (img.general ? 1e15 : 0) + -new Date(img.createdAt ?? 0).getTime(),
        },
      })),
    [filtered]
  );
  ...
  const rowActions = useCallback(
    (id: string) => {
      ...
      if (!img || img.general) return <span />;
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" sideOffset={4}>
            {isInactive && (
              <DropdownMenuItem onClick={() => handleActivate(img)} disabled={loadingImages[id]}>
  ...
  return (
    <>
      <Resource
        ...
        create={{ label: "New image", onClick: () => setShowCreateDialog(true) }}
        defaultSort="created"
      ...
      >
        <ModalContent size="sm">
          ...
          <ModalFooter>
            ...
            >
              {imageToDelete && loadingImages[imageToDelete.id] ? "Deleting..." : "Delete"}
            </button>
          </ModalFooter>
        </ModalContent>
      </Modal>

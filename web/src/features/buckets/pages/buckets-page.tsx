...
import { type Bucket, BucketState } from '@snapflow/api-client';
import { Database, MoreVertical, Trash2 } from 'lucide-react';
import { useCallback, useEffect, useMemo, useState } from 'react';
import { apiClient } from '@/api/api-client';
import { CreateBucketModal } from '@/features/buckets/components/create-bucket-modal';
...
export default function BucketsPage() {
	...
	const [processingBucketAction, setProcessingBucketAction] = useState<
	...
	const fetchBuckets = useCallback(
		async (showTableLoadingState = true) => {
			...
			try {
				const data = (await bucketApi.listBuckets(selectedOrganization.id))
					.data;
				setBuckets(data);
			} catch (error) {
	...
	useEffect(() => {
		...
		const handleBucketStateUpdatedEvent = (data: {
			...
			if (data.newState === BucketState.DELETED) {
				...
				setBuckets((prev) =>
					prev.map((v) => (v.id === data.bucket.id ? data.bucket : v))
				);
			}
		};
		...
		const handleBucketLastUsedAtUpdatedEvent = (bucket: Bucket) => {
			if (!buckets.some((v) => v.id === bucket.id)) {
				...
				setBuckets((prev) =>
					prev.map((v) => (v.id === bucket.id ? bucket : v))
				);
			}
		};
		...
		realtimeSocket.on(
			'bucket.lastUsedAt.updated',
			handleBucketLastUsedAtUpdatedEvent
		);
	...
	const handleDelete = async (bucket: Bucket) => {
		...
		setBuckets((prev) =>
			prev.map((v) =>
				v.id === bucket.id ? { ...v, state: BucketState.PENDING_DELETE } : v
			)
		);
		try {
			...
			setBuckets((prev) =>
				prev.map((v) =>
					v.id === bucket.id ? { ...v, state: bucket.state } : v
				)
			);
		} finally {

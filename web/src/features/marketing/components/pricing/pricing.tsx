		...
		limits: [
			{
				...
				value: fmtLimit(bt.maxConcurrentSandboxes),
			},
			{
				label: 'Max sandbox lifetime',
				value: fmtLife(bt.maxSandboxLifetimeSeconds),
			},
...
const COMPUTE_RATES: UsageRow[] = [
	{
		resource: 'vCPU',
		price: formatRate(HOURLY_RATES.CPU),
		unit: 'core / hour',
	},
	{
		resource: 'Memory',
		price: formatRate(HOURLY_RATES.MEMORY),
		unit: 'GiB / hour',
	},
	{
		resource: 'Disk',
		price: formatRate(HOURLY_RATES.DISK),
		unit: 'GiB / hour',
	},
		...
		<div className="flex flex-col rounded-lg border border-[#E5E5E5] bg-white">
			...
			<div className="flex flex-1 flex-col border-[#E5E5E5] border-t">
				{tier.limits.map((limit, i) => (
					...
					>
						<span className="font-season text-[#5c5c5c] text-[13px]">
							{limit.label}
						</span>
						<span className="font-[430] font-season text-[#1C1C1C] text-[13px]">
							{limit.value}
						</span>
					</div>
				))}
	...
	return (
		<div className="overflow-hidden rounded-lg border border-[#E5E5E5] bg-white">
			<div className="border-[#E5E5E5] border-b px-5 py-3">
				<h3 className="font-[430] font-season text-[#1C1C1C] text-[15px]">
					{title}
				</h3>
			</div>
				...
				{rows.map((row) => (
					...
					>
						<div className="flex items-center gap-2.5">
							...
							{row.spec && (
								<span className="font-season text-[#999] text-[12px]">
									{row.spec}
								</span>
							)}
						</div>
						<div className="flex items-baseline gap-1">
							<span className="font-mono text-[#1C1C1C] text-[14px]">
								{row.price}
							</span>
							<span className="font-season text-[#999] text-[12px]">
								{row.unit}
			...
			<div className="px-4 pt-20 pb-20 sm:px-8 md:px-20">
				<div className="flex flex-col items-start gap-3 sm:gap-4 md:gap-5">
					...
					<p className="max-w-140 font-[430] font-season text-[#5c5c5c] text-[15px] leading-[150%] tracking-[0.02em] lg:text-[17px]">
						Add funds to your organization wallet and only pay for the compute
						you consume. Higher wallet balances unlock higher limits
						automatically.
					</p>
				</div>
				...
				<div className="mt-16">
					...
					<p className="mt-2 font-[430] font-season text-[#5c5c5c] text-[14px] leading-[150%]">
						Billed per second. All sandbox time is deducted from your wallet
						balance.
					</p>
					...
					<p className="mt-4 font-season text-[#999] text-[12px] leading-[150%]">
						Per-second billing with 1-minute minimum. Prices shown exclude
						applicable taxes. Persistent storage billed at rest even when
						sandboxes are stopped.
					</p>
				</div>
			</div>
		</section>

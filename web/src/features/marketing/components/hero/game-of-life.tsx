...
function seedPatterns(grid: Uint8Array, cols: number, rows: number) {
	...
	for (let i = 0; i < 3; i++) {
		place(randX(), randY(), rPentomino);
	}
	for (let i = 0; i < 4; i++) {
		place(randX(), randY(), glider);
	}
	for (let i = 0; i < 2; i++) {
		place(randX(), randY(), lwss);
	}
...
function buildColorMap(cols: number, rows: number): Uint8Array {
	...
	const seeds: { x: number; y: number; hue: number }[] = anchors.map(
		([ax, ay], i) => ({
...
export function GameOfLife({
	...
	const drawGrid = useCallback(
		(
			...
			if (!colorMap || !intensityMap || !sizeMap || !opacityMap) {
				return;
			}
			...
			if (target && intensityMap && afterglow) {
				for (let i = 0; i < target.length; i++) {
					...
					if (currentGrid[i]) {
						...
						afterglow[i]--;
						target[i] =
							maxOpacity * intensity * 0.4 * (afterglow[i] / AFTERGLOW_TICKS);
					} else {
						target[i] = 0;
						afterglow[i] = 0;
					}

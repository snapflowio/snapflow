...
import {
	type Organization,
	type OrganizationRolePermissionsEnum,
...
import {
	...
	type SelectedOrganizationContextImplementation,
...
export function SelectedOrganizationProvider(props: Props) {
	...
	>(() => {
		...
		if (organizations.length > 0) {
			const defaultOrg =
				organizations.find((org) => org.personal) || organizations[0];
	...
	const getOrganizationMembers = useCallback(
		async (selectedOrganizationId: string | null) => {
			...
			try {
				return (
					await organizationsApi.listOrganizationMembers(selectedOrganizationId)
				).data;
			} catch (error) {
	...
	const authenticatedUserOrganizationMember = useMemo(() => {
		return (
			organizationMembers.find((member) => member.userId === user?.id) || null
		);
	...
	const authenticatedUserAssignedPermissions = useMemo(() => {
		...
		return new Set(
			authenticatedUserOrganizationMember.assignedRoles.flatMap(
	...
	const authenticatedUserHasPermission = useCallback(
		(permission: OrganizationRolePermissionsEnum) => {
			if (
				!authenticatedUserOrganizationMember ||
				!authenticatedUserAssignedPermissions
			) {
				return false;
			}
	...
	const contextValue: SelectedOrganizationContextImplementation =
		useMemo(() => {
			return {

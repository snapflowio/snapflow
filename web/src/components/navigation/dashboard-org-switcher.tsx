...
export function DashboardOrgSwitcher({ collapsed }: DashboardOrgSwitcherProps) {
  ...
  const handleCreate = async (name: string) => {
    try {
      const org = (await organizationsApi.createOrganization({ name: name.trim() })).data;
            ...
            <div className="flex max-h-32.5 flex-col gap-0.5 overflow-y-auto">
              {sortedOrgs.map((org) => (
                <div
                  key={org.id}
                  className={cn(
                    "group flex cursor-pointer select-none items-center gap-2 rounded-[5px] px-2 py-1.25 font-medium text-[12px] text-text-body outline-none transition-colors hover:bg-surface-active",

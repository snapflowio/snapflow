...
import { OrganizationRolePermissionsEnum, OrganizationUserRoleEnum } from "@snapflow/api-client";
import {
  BookOpen,
  Box,
  Container,
...
function SidebarNavItem({ icon: Icon, label, href, onClick, isActive }: SidebarNavItemProps) {
  const cls = cn(
...
export function DashboardSidebar({ collapsed, onToggleCollapse }: DashboardSidebarProps) {
  ...
  const isActive = (href: string) => {
    if (href === Path.DASHBOARD) return location.pathname === href;

 ...
 * ```tsx
 * import { Popover, PopoverAnchor, PopoverBackButton, PopoverContent, PopoverFolder, PopoverItem } from '@/components/ui'
...
const STYLES = {
	...
	size: {
		...
		PopoverSize,
		{ item: string; icon: string; section: string }
	>,
	...
	colorScheme: {
		default: {
			...
			section: 'text-(--text-tertiary)',
			search: 'text-(--text-muted)',
			searchInput: 'text-(--text-primary) placeholder:text-(--text-muted)',
			content: 'bg-(--surface-5) text-foreground dark:bg-(--surface-3)',
			divider: 'border-(--border-1)',
		},
		inverted: {
			text: 'text-white dark:text-(--text-primary)',
			section: 'text-(--text-muted-inverse)',
			search: 'text-(--text-muted-inverse) dark:text-(--text-muted)',
			searchInput:
				'text-white placeholder:text-(--text-muted-inverse) dark:text-(--text-primary) dark:placeholder:text-(--text-muted)',
			content:
				'bg-[#1b1b1b] text-white dark:bg-(--surface-3) dark:text-foreground',
			divider: 'border-[#363636] dark:border-(--border-1)',
		},
	} satisfies Record<
		PopoverColorScheme,
		{
	...
	states: {
		default: {
			active:
				'bg-(--border-1) text-(--text-primary) [&_svg]:text-(--text-primary)',
			hover:
				'hover:bg-(--border-1) hover:text-(--text-primary) hover:[&_svg]:text-(--text-primary)',
		},
		secondary: {
			active: 'bg-(--brand-secondary) text-white [&_svg]:text-white',
			hover:
				'hover:bg-(--brand-secondary) hover:text-white hover:[&_svg]:text-white',
		},
		inverted: {
			active:
				'bg-[#363636] text-white [&_svg]:text-white dark:bg-(--surface-5) dark:text-(--text-primary) dark:[&_svg]:text-(--text-primary)',
			hover:
				'hover:bg-[#363636] hover:text-white hover:[&_svg]:text-white dark:hover:bg-(--surface-5) dark:hover:text-(--text-primary) dark:hover:[&_svg]:text-(--text-primary)',
		},
	},
} as const;
...
function getItemStateClasses(
	...
	return colorScheme === 'inverted'
		? STYLES.states.inverted[state]
		: STYLES.states.default[state];
}
...
const Popover: React.FC<PopoverProps> = ({
	...
	const [onFolderSelect, setOnFolderSelect] = React.useState<
		(() => void) | null
	>(null);
	const [searchQuery, setSearchQuery] = React.useState<string>('');
	const [lastHoveredItem, setLastHoveredItem] = React.useState<string | null>(
		null
	);
			...
			<PopoverPrimitive.Content
				...
				className={cn(
					...
					hasUserWidthConstraint &&
						'**:data-popover-section:truncate [&_.flex-1]:truncate',
				...
				style={{
					...
					maxWidth:
						maxWidth !== undefined ? `${maxWidth}px` : 'calc(100vw - 16px)',
					minWidth:
						minWidth !== undefined
							? `${minWidth}px`
							: hasUserWidthConstraint
...
export interface PopoverScrollAreaProps
	extends React.HTMLAttributes<HTMLDivElement> {}

/**
 * Scrollable container for popover items.
 */
const PopoverScrollArea = React.forwardRef<
	HTMLDivElement,
...
const PopoverItem = React.forwardRef<HTMLDivElement, PopoverItemProps>(
	(
		...
		React.useEffect(() => {
			...
			const content = itemRef.current.closest(
		...
		const isActive =
			active !== undefined
				? active
				: itemIndex >= 0 && context?.selectedIndex === itemIndex;
		...
		return (
			...
			>
				{children}
				{showCheck && (
					<Check className={cn('ml-auto', STYLES.size[size].icon)} />
				)}
			</div>
		);
	}
...
export interface PopoverFolderProps
...
const PopoverFolder = React.forwardRef<HTMLDivElement, PopoverFolderProps>(
	(
		...
		React.useEffect(() => {
			...
			const content = triggerRef.current.closest(
				'[data-radix-popper-content-wrapper]'
			);
			if (!content) {
				return;
		...
		const handleMouseEnter = () => {
			...
			if (triggerRef.current) {
				...
				const parentPopover = triggerRef.current.closest(
					'[data-radix-popper-content-wrapper]'
				);
		...
		const isActive =
			active !== undefined
				? active
				: itemIndex >= 0 && selectedIndex === itemIndex;
...
export interface PopoverBackButtonProps
	extends React.HTMLAttributes<HTMLDivElement> {
 ...
 */
const PopoverBackButton = React.forwardRef<
	HTMLDivElement,
	PopoverBackButtonProps
>(
	(
		{
			className,
...
const PopoverSearch = React.forwardRef<HTMLDivElement, PopoverSearchProps>(
	({ className, placeholder = 'Search...', onValueChange, ...props }, ref) => {
		const { searchQuery, setSearchQuery, size, colorScheme } =
			usePopoverContext();
...
export interface PopoverDividerProps
	extends React.HTMLAttributes<HTMLDivElement> {
	/** Only show when not inside any folder */
	rootOnly?: boolean;
}
	...
	({ className, rootOnly, ...props }, ref) => {
		...
		return (
			<div
				...
				className={cn(
					'my-1.5 border-t',
					STYLES.colorScheme[colorScheme].divider,
					className
				)}
				role="presentation"
				{...props}

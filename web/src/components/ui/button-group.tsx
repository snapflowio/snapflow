...
function ButtonGroup({
	...
	return (
		<ButtonGroupContext.Provider value={{ value, onValueChange, disabled }}>
			<div className={cn(buttonGroupVariants({ gap }), className)} {...props}>
...
const buttonGroupItemVariants = cva(
	'inline-flex items-center justify-center font-medium transition-colors outline-none focus:outline-none focus-visible:outline-none disabled:pointer-events-none disabled:opacity-70 px-[8px] py-[4px] text-[12px] border',
	{
		variants: {
			active: {
				true: 'bg-[#1D1D1D] text-(--text-inverse) border-[#1D1D1D] hover:bg-[#2A2A2A] hover:border-[#2A2A2A] dark:bg-white dark:border-white dark:hover:bg-[#E0E0E0] dark:hover:border-[#E0E0E0]',
				false:
					'bg-(--surface-4) text-(--text-secondary) border-(--border) hover:text-(--text-primary) hover:bg-(--surface-6) hover:border-(--border-1)',
			},
			position: {
				only: 'rounded-[5px]',
...
function ButtonGroupItem({
	...
	return (
		<button
			...
			onClick={handleClick}
			className={cn(
				buttonGroupItemVariants({ active: isActive, position: _position }),
				className
			)}
			{...props}
		>

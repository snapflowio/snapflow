 ...
 * @example
 * ```tsx
 * import { TagInput, type TagItem } from '@/components/ui'
 *
  ...
  "flex w-auto cursor-default items-center gap-[3px] rounded-[4px] px-[4px] font-medium font-sans text-[13px] leading-[20px] transition-colors",
  {
    variants: {
      variant: {
        ...
        secondary:
          "border border-(--border-1) bg-(--surface-4) text-(--text-secondary) hover:text-(--text-primary)",
        invalid: "bg-[#fecaca] text-(--text-error) dark:bg-[#551a1a] dark:text-(--text-error)",
      },
    },
    defaultVariants: {
      variant: "default",
...
const Tag = React.memo(function Tag({
  ...
  return (
    <div className={cn(tagVariants({ variant }), className)}>
      <span className="max-w-50 truncate">{value}</span>
      {suffix}
      {!disabled && onRemove && (
        <button
          type="button"
          onClick={onRemove}
          className={cn(
            "shrink-0 opacity-80 transition-opacity hover:opacity-100 focus:outline-none",
            variant === "invalid"
              ? "text-text-error"
              : variant === "secondary"
                ? "text-text-tertiary"
...
const tagInputVariants = cva(
  "scrollbar-hide flex w-full cursor-text flex-wrap items-center gap-x-[8px] gap-y-[4px] overflow-y-auto rounded-[4px] border border-(--border-1) bg-(--surface-5) px-[8px] py-[6px] transition-colors focus-within:outline-none dark:bg-(--surface-5)",
  {
    variants: {
      variant: {
        default: "",
    ...
    return (
      <div
        className={cn(
          ...
          isDragging && "border-border border-dashed bg-surface-5",
          className
        )}
      ...
      >
        ...
        {isDragging && (
          <div className="absolute inset-0 flex items-center justify-center rounded-sm bg-(--surface-5)/90">
            <span className="text-[13px] text-text-tertiary">Drop file here</span>
          </div>
        )}
        ...
        >
          <div className="relative inline-flex">
            {inputValue.trim() && (
              <span
                className="invisible whitespace-pre font-medium font-sans text-[13px] leading-5"
                aria-hidden="true"
              >
                {inputValue}
              </span>
            ...
            <input
              ...
              className={cn(
                "border-none bg-transparent font-medium font-sans outline-none placeholder:text-text-muted disabled:cursor-not-allowed disabled:opacity-50",
                inputValue.trim()
                  ? "absolute top-0 left-0 h-full w-full p-0 text-[13px] text-inherit leading-5"
          ...
          {inputValue.trim() && (
            <button
              ...
              className="flex items-center px-0.75 opacity-80 transition-opacity hover:opacity-100 focus:outline-none"
              disabled={disabled}
              aria-label="Add tag"
            >
          ...
          <Tooltip.Root>
            <Tooltip.Trigger asChild>
              <button
                ...
                className="absolute right-2 bottom-2.25 text-text-tertiary transition-colors hover:text-text-secondary"

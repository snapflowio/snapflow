...
>(({ index, className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative flex h-12 w-12 items-center justify-center rounded-[4px] border border-(--border-1) bg-(--surface-5) font-medium text-(--text-primary) text-lg transition-colors",
...
const InputOTPSeparator = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<"div">
>(({ ...props }, ref) => (
  <div ref={ref} role="separator" className="text-(--text-muted)" {...props}>
    <Minus />
  </div>
));

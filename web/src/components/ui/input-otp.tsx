...
>(({ index, className, ...props }, ref) => {
	...
	return (
		<div
			ref={ref}
			className={cn(
				'relative flex h-12 w-12 items-center justify-center rounded-sm border border-border-1 bg-surface-5 font-medium text-text-primary text-lg transition-colors',
				isActive && 'z-10 border-text-muted ring-text-muted ring-1',
				className
			)}
			{...props}
		>
			...
			{hasFakeCaret && (
				<div className="pointer-events-none absolute inset-0 flex items-center justify-center">
					<div className="h-6 w-px animate-caret-blink bg-text-primary duration-1000" />
				</div>
			)}
		</div>
	);
...
>(({ ...props }, ref) => {
	return (
		<div ref={ref} role="presentation" className="text-text-muted" {...props}>
			<Minus />
		</div>
	);
});

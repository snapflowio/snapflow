...
import { Popover, PopoverAnchor, PopoverContent, PopoverScrollArea } from "./popover";
          ...
          <div ref={containerRef} className="relative w-full" {...props}>
                ...
                {editable ? (
                  <div className="group relative">
                    <Input
                      ref={inputRef}
                      className={cn(
                        "w-full pr-[40px] font-medium transition-colors hover:bg-(--surface-7) dark:hover:border-(--surface-7) dark:hover:bg-(--border-1)",
                    ...
                    {(overlayContent || SelectedIcon) && (
                      ...
                      >
                        {overlayContent ? (
                          ...
                          <>
                            {SelectedIcon && <SelectedIcon className="mr-[8px] h-3 w-3 shrink-0" />}
                    ...
                    <div
                      className="absolute top-1/2 right-[4px] z-10 flex h-6 w-6 -translate-y-1/2 cursor-pointer items-center justify-center"
            ...
            <PopoverContent
              ...
              className={cn(
                ...
                dropdownWidth === "trigger" && "w-(--radix-popover-trigger-width)"
              )}
              style={
            ...
            >
              {searchable && (
                <div className="flex items-center px-[10px] pt-[8px] pb-[4px]">
                  <Search className="mr-[7px] ml-px h-[13px] w-[13px] shrink-0 text-(--text-muted)" />
                  <input
                    ref={searchInputRef}
                ...
                <div ref={dropdownRef} role="listbox" id={listboxId}>
                  {isLoading ? (
                    <div className="flex items-center justify-center py-[14px]">
                      <Loader2 className="h-[16px] w-[16px] animate-spin text-(--text-muted)" />
                      <span className="ml-[8px] font-base text-(--text-muted) text-[12px]">
                        Loading options...
                      </span>
                    </div>
                  ) : error ? (
                    ...
                    <div className="py-[14px] text-center font-base text-(--text-muted) text-[12px]">
                      {emptyMessage ||
                      ...
                      {filteredGroups.map((group, groupIndex) => (
                        <div key={group.section || `group-${groupIndex}`}>
                          {group.sectionElement
                            ? group.sectionElement
                            : group.section && (
                                <div className="px-[6px] py-[4px] font-base text-(--text-tertiary) text-[11px] first:pt-[4px]">
                                  {group.section}
                                </div>
                              )}
                            ...
                            return (
                              ...
                              >
                                {option.iconElement
                                  ? option.iconElement
                                  : OptionIcon && (
                                      <OptionIcon className="h-[14px] w-[14px] shrink-0" />
                                    )}
                                ...
                                {multiSelect && isSelected && (
                                  <Check className="ml-[8px] h-[12px] w-[12px] shrink-0 text-(--text-primary)" />
                                )}
                              </div>
                            );
                          })}
                      ...
                      {filteredOptions.map((option, index) => {
                        ...
                        return (
                          ...
                          >
                            {option.iconElement
                              ? option.iconElement
                              : OptionIcon && <OptionIcon className="h-[14px] w-[14px] shrink-0" />}
                            <span className="flex-1 truncate text-(--text-primary)">
                              {option.label}
                            </span>
                            {option.suffixElement}

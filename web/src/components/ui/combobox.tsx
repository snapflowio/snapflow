...
const comboboxVariants = cva(
	'flex w-full rounded-[4px] border border-(--border-1) bg-(--surface-5) px-[8px] font-sans font-medium text-(--text-primary) placeholder:text-(--text-muted) outline-none focus:outline-none focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50 hover:bg-(--surface-7) dark:hover:border-(--surface-7) dark:hover:bg-(--border-1)',
	{
		variants: {
			variant: {
				default: '',
		...
		(
			...
			const filteredOptions = useMemo(() => {
				...
				if (filterOptions && value && open) {
					...
					const exactMatch = result.find(
						(opt) =>
							opt.value === value || opt.label.toLowerCase() === currentValue
					);
					if (!exactMatch) {
						result = result.filter((option) => {
							...
							return (
								label.includes(currentValue) ||
								optionValue.includes(currentValue)
							);
						});
					}
				}
			...
			const handleBlur = useCallback(() => {
				...
				setTimeout(() => {
					...
					if (
						!activeElement ||
						(!isInContainer && !isInDropdown && !isSearchInput)
					) {
						setOpen(false);
						setHighlightedIndex(-1);
						setSearchQuery('');
								...
								{editable ? (
									<div className="group relative">
										<Input
											ref={inputRef}
											className={cn(
												'w-full pr-10 font-medium transition-colors hover:bg-surface-7 dark:hover:border-surface-7 dark:hover:bg-border-1',
										...
										{(overlayContent || SelectedIcon) && (
											<div
												className={cn(
													'pointer-events-none absolute top-0 right-10.5 bottom-0 left-0 flex items-center bg-transparent px-2 py-1.5 font-medium font-sans text-sm',
													disabled && 'opacity-50'
												)}
											>
												{overlayContent ? (
													...
													<>
														{SelectedIcon && (
															<SelectedIcon className="mr-2 h-3 w-3 shrink-0" />
														)}
										...
										<div
											className="absolute top-1/2 right-1 z-10 flex h-6 w-6 -translate-y-1/2 cursor-pointer items-center justify-center"
									...
									>
										...
										/>
										{overlayContent && (
											<div className="pointer-events-none absolute inset-y-0 right-6 left-0 flex items-center px-2">
						...
						>
							{searchable && (
								<div className="flex items-center px-2.5 pt-2 pb-1">
									<Search className="mr-1.75 ml-px h-3.25 w-3.25 shrink-0 text-text-muted" />
									<input
										ref={searchInputRef}
										className="w-full bg-transparent font-base text-text-primary text-[13px] placeholder:text-text-muted focus:outline-none"
								...
								<div ref={dropdownRef} role="listbox" id={listboxId}>
									{isLoading ? (
										<div className="flex items-center justify-center py-3.5">
											...
											<span className="ml-2 font-base text-text-muted text-[12px]">
												Loading options...
											</span>
										</div>
									) : error ? (
										<div className="px-1.5 py-3.5 text-center font-base text-[12px] text-red-500">
											{error}
										</div>
									) : filteredOptions.length === 0 ? (
										<div className="py-3.5 text-center font-base text-text-muted text-[12px]">
											{emptyMessage ||
											...
											{filteredGroups.map((group, groupIndex) => (
												<div key={group.section || `group-${groupIndex}`}>
													{group.sectionElement
														? group.sectionElement
														: group.section && (
																<div className="px-1.5 py-1 font-base text-text-tertiary text-[11px] first:pt-1">
																	{group.section}
																</div>
															)}
														...
														return (
															...
															>
																...
																{multiSelect && isSelected && (
																	<Check className="ml-2 h-3 w-3 shrink-0 text-text-primary" />
																)}
															</div>
														);
													})}
												...
												<div
													...
													className={cn(
														...
														size === 'sm'
															...
															: 'py-1.5 text-sm',
														'hover:bg-border-1',
														!multiSelectValues?.length && 'bg-border-1'
													)}
												>
													<span className="flex-1 truncate text-text-primary">
														{allOptionLabel}
													...
													<div
														...
														className={cn(
															...
															size === 'sm'
																...
																: 'py-1.5 text-sm',
															'hover:bg-border-1',
															(isHighlighted || isSelected) && 'bg-border-1',

...
import { Modal, type ModalContentProps, ModalOverlay, ModalPortal } from "./modal";
    ...
    <ModalPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          "fixed top-[50%] z-500 flex h-[min(calc(100vh-8rem),900px)] min-h-[400px] w-[min(calc(100vw-8rem),1080px)] min-w-[520px] translate-x-[-50%] translate-y-[-50%] flex-row rounded-[8px] border bg-(--bg) shadow-sm duration-200",
          className
        )}
        style={{
          left: "50%",
...
const SModalSidebarHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("mb-[16px] px-[12px] font-medium text-(--text-primary) text-[16px]", className)}
      {...props}
    />
  )
);
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("mb-[2px] font-medium text-(--text-muted) text-[12px]", className)}
    {...props}
  />
));
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        !active &&
          "text-(--text-tertiary) hover:bg-(--surface-6) hover:text-(--text-primary) dark:hover:bg-(--border)",
        className
      )}
      {...props}
    >
      {icon && (
        <span className="h-[14px] w-[14px] shrink-0 [&>svg]:h-full [&>svg]:w-full">{icon}</span>
      )}
      {children}
    </Button>
  );
...
const SModalMain = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "flex min-w-0 flex-1 flex-col gap-[16px] overflow-hidden rounded-[8px] border-l bg-(--surface-2) p-[14px]",
        className
      )}
      {...props}
    />
...
const SModalMainHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, children, ...props }, ref) => (
    <div ref={ref} className={cn("flex items-center justify-between", className)} {...props}>
      <span className="font-base text-(--text-muted) text-[14px]">{children}</span>
...
>(({ className, children, activeValue, disabled = false, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "pointer-events-none absolute bottom-0 h-px rounded-full bg-(--text-primary)",
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ref={ref}
    className={cn(
      "px-1 pb-[8px] font-medium text-(--text-secondary) text-[13px] transition-colors",
...
const SModalTabsBody = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "min-h-0 flex-1 overflow-y-auto border-(--border) border-t pt-[10px]",
        className
      )}
      {...props}
    />

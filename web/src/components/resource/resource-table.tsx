...
function ResourceColGroup({ columns }: { columns: ResourceColumn[] }) {
	const totalWeight = columns.reduce(
		(sum, col) => sum + (col.widthMultiplier ?? 1),
		0
	);
	return (
		<colgroup>
...
function CellContent({
	...
	return (
		...
		>
			{cell.icon && (
				<span className="shrink-0 text-text-icon">{cell.icon}</span>
			)}
						...
						<tr>
							{columns.map((col) => {
								...
								return (
									<th
										key={col.id}
										className="h-10 px-4 py-1.5 text-left align-middle"
									>
										<Button
											variant="subtle"
										...
										>
											{col.header}
											{isActive && (
												<SortIcon className="ml-1 h-3 w-3 text-text-icon" />
											)}
										</Button>
									</th>
								);
						...
						{create && (
							...
							>
								<td
									...
									className="px-6 py-2.5 align-middle"
								>

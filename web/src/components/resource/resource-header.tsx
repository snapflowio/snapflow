    ...
    <div className="shrink-0 border-border border-b px-6 py-2.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          {title && <h1 className="font-medium text-[14px] text-text-body">{title}</h1>}
        </div>
        <div className="flex items-center gap-1.5">
          {actions?.map((action) => {
            ...
            return (
              ...
              >
                {ActionIcon && (
                  <ActionIcon
                    className={cn("h-3.5 w-3.5 text-text-icon", action.label && "mr-1.5")}
                  />
                )}
                {action.label}
              </Button>
          ...
          {create && (
            <Button variant="subtle" size="sm" onClick={create.onClick} disabled={create.disabled}>

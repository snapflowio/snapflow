...
import { cn, formatDate, placeholderBlurhash } from "@/lib/util";
    ...
    >
      ...
      >
        <div className="w-full">
          <h2 className="my-1.5 line-clamp-2 font-bold text-2xl">{data.title}</h2>
          {data.description && (
        ...
        <div className="mt-4 flex items-center space-x-3">
          ...
          {data.date && <p className="text-muted-foreground text-sm">{formatDate(data.date)}</p>}
        </div>
      </div>

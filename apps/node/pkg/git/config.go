...
func (s *Service) SetGitConfig(userData *provider.GitUser, providerConfig *provider.GitProviderConfig) error {
	...
	_, err = cfg.Section("credential").NewKey("helper", "/usr/local/bin/snapflow git-cred")
	if err != nil {
		return err
	}
...
func (s *Service) setSigningConfig(cfg *ini.File, providerConfig *provider.GitProviderConfig, userData *provider.GitUser) error {
	...
	case provider.SigningMethodGPG:
		_, err := cfg.Section("commit").NewKey("gpgSign", "true")
		if err != nil {
			return err
		}
	case provider.SigningMethodSSH:

...
func (p *Proxy) AuthCallback(ctx *gin.Context) {
	...
	ctx.SetCookie(SNAPFLOW_SANDBOX_AUTH_COOKIE_NAME+sandboxId, encoded, 3600, "/", cookieDomain, p.config.EnableTLS, true)
...
func (p *Proxy) hasSandboxAccess(ctx context.Context, sandboxId string, authToken string) bool {
	...
	return res != nil //&& res.StatusCode == http.StatusOK Figure out why this is erroring
}

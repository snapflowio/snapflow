...
export class Migration1751784196393 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."sandbox_region_enum" AS ENUM('eu', 'us', 'asia')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."sandbox_class_enum" AS ENUM('small', 'medium', 'large')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."sandbox_backupstate_enum" AS ENUM('None', 'Pending', 'InProgress', 'Completed', 'Error')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "build_info" ("snapshotRef" character varying NOT NULL, "dockerfileContent" text, "contextHashes" text, "lastUsedAt" TIMESTAMP NOT NULL DEFAULT now(), "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "build_info_snapshotRef_pk" PRIMARY KEY ("snapshotRef"))`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."runner_class_enum" AS ENUM('small', 'medium', 'large')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."runner_region_enum" AS ENUM('eu', 'us', 'asia')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "snapshot" DROP CONSTRAINT "snapshot_buildInfoSnapshotRef_fk"`,
    );
    await queryRunner.query(
      `ALTER TABLE "sandbox" DROP CONSTRAINT "sandbox_buildInfoSnapshotRef_fk"`,
    );
    await queryRunner.query(`DROP TABLE "docker_registry"`);
    await queryRunner.query(
      `DROP TYPE "public"."docker_registry_registrytype_enum"`,
    );

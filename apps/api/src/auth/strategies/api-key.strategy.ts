...
import { ProxyContext } from "../../common/interfaces/proxy-content.interface";
  ...
  implements OnModuleInit
{
  ...
  async validate(token: string): Promise<AuthContext | ProxyContext> {
    ...
    if (proxyApiKey === token) return { role: "proxy" };
    ...
    try {
      ...
      if (!user) throw new UnauthorizedException("This API key is invalid");

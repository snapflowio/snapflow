...
export class RunnerService {
  ...
  async getRandomAvailableRunner(params: GetRunnerParams): Promise<Runner> {
    ...
    if (availableRunners.length === 0) throw new BadRequestError("No available runners");
  ...
  async getRunnersBySnapshotInternalName(internalName: string): Promise<RunnerSnapshotDto[]> {
    ...
    this.logger.debug(
      `Found ${snapshotRunners.length} snapshot runners for snapshot ${snapshot.id}`
    );
    ...
    this.logger.debug(
      `Found ${runners.length} runners with IDs: ${runners.map((r) => r.id).join(", ")}`
    );

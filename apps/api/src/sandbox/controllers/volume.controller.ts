...
export class VolumeController {
  ...
  async createVolume(
    ...
    let concurrentCreateCount =
      Number.parseInt(await this.redis.get(concurrentCreateKey)) || 0;
    concurrentCreateCount++;
    await this.redis.setex(concurrentCreateKey, 1, concurrentCreateCount);

    const activeVolumeCount = await this.volumeService.countActive(
      organization.id,
    );
  ...
  async getVolumeByName(
    ...
    const volume = await this.volumeService.findByName(
      authContext.organizationId,
      name,
    );
    return VolumeDto.fromVolume(volume);

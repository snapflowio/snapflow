...
import { ImageEvents } from "../constants/image-events.constants";
...
export class ImageSubscriber implements EntitySubscriberInterface<Image> {
      ...
      switch (column) {
        case "enabled":
          this.eventEmitter.emit(
            ImageEvents.ENABLED_TOGGLED,
            new ImageEnabledToggledEvent(event.entity as Image)
          );
          break;
        case "state":
          this.eventEmitter.emit(
            ImageEvents.STATE_UPDATED,
            new ImageStateUpdatedEvent(
              event.entity as Image,
  ...
  beforeRemove(event: RemoveEvent<Image>) {
    this.eventEmitter.emit(
      ImageEvents.REMOVED,
      new ImageRemovedEvent(event.databaseEntity as Image)
    );
  }
}

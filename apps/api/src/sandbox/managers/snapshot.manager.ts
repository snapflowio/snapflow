...
export class SnapshotManager {
      ...
      runnerSnapshots.map((snapshotRunner) => {
        return this.syncRunnerSnapshotState(snapshotRunner).catch((err) => {
          if (err.code !== "ECONNRESET") {
            ...
            this.logger.error(
              `Error syncing runner snapshot state ${snapshotRunner.id}: ${fromAxiosError(err)}`
            );
  ...
  async propagateSnapshotToRunners(internalSnapshotName: string) {
    ...
    try {
      ...
      const propagateLimit =
        Math.floor(runners.length / 3) - snapshotRunnersDistinctRunnersIds.length;
      const unallocatedRunners = runners.filter(
        (runner) =>
          !snapshotRunnersDistinctRunnersIds.some(
            (snapshotRunnerId) => snapshotRunnerId === runner.id
          )
      );
        ...
        runnersToPropagateTo.map(async (runner) => {
          ...
          try {
            if (snapshotRunner && !snapshotRunner.snapshotRef) {
              ...
              this.logger.warn(
                `Internal snapshot name not found for snapshot runner ${snapshotRunner.id}`
              );
              return;
            }
            ...
            this.logger.error(
              `Error propagating snapshot to runner ${runner.id}: ${fromAxiosError(err)}`
            );
  ...
  async propagateSnapshotToRunner(internalSnapshotName: string, runner: Runner) {
    let dockerRegistry =
      await this.dockerRegistryService.findOneBySnapshotImageName(internalSnapshotName);
    ...
    if (!dockerRegistry) {
      ...
      if (!dockerRegistry) {
        throw new Error(
          "No registry found for snapshot and no default internal registry configured"
        );
      }
    }
  ...
  async handleSnapshotStateBuilding(snapshot: Snapshot) {
    ...
    if (Date.now() - snapshot.createdAt.getTime() > timeoutMs) {
      await this.updateSnapshotState(
        snapshot.id,
        SnapshotState.BUILD_FAILED,
        "Timeout while building snapshot"
      );
      return;
    }
  ...
  async handleSnapshotStatePulling(snapshot: Snapshot) {
    ...
    if (Date.now() - snapshot.createdAt.getTime() > timeoutMs) {
      await this.updateSnapshotState(
        snapshot.id,
        SnapshotState.ERROR,
        "Timeout while pulling snapshot"
      );
      return;
    }
  ...
  private async updateSnapshotState(
    snapshotId: string,
    state: SnapshotState,
    errorReason?: string
  ) {
  ...
  async cleanupOldBuildInfoSnapshotRunners() {
    ...
    try {
      ...
      if (result.affected > 0) {
        this.logger.debug(
          `Marked ${result.affected} SnapshotRunners for removal due to unused BuildInfo`
        );
      }
    } catch (error) {
      this.logger.error(
        `Failed to mark old BuildInfo SnapshotRunners for removal: ${fromAxiosError(error)}`
      );
    } finally {
      await this.redisLockProvider.unlock(lockKey);
    }
  ...
  async deactivateOldSnapshots() {
    ...
    try {
      ...
      const internalNames = oldSnapshots
        .map((snapshot) => snapshot.internalName)

...
/// struct for typed errors of method [`oauth_exchange_token`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum OauthExchangeTokenError {
    Status400(),
    UnknownValue(serde_json::Value),
}


pub async fn oauth_authorize(configuration: &configuration::Configuration, authorize_request: models::AuthorizeRequest, x_snapflow_organization_id: Option<&str>) -> Result<models::AuthorizeResponse, Error<OauthAuthorizeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_authorize_request = authorize_request;
    ...
    req_builder = req_builder.json(&p_authorize_request);
...
pub async fn oauth_exchange_token(configuration: &configuration::Configuration, token_request: models::TokenRequest, x_snapflow_organization_id: Option<&str>) -> Result<models::TokenResponse, Error<OauthExchangeTokenError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<OauthExchangeTokenError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

...
import { TOCFooter } from "@/components/layout/footer";
import { PageNavigationArrows } from "@/components/layout/pagination-arrows";
...
const BASE_URL = "https://snapflow.io";
    ...
    <div className="mt-12">
      <div className="flex items-center justify-between py-8">
        {rawNeighbours?.previous ? (
          <Link
            href={rawNeighbours.previous.url}
            className="group flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
          >
            <ChevronLeft className="h-4 w-4 transition-transform group-hover:-translate-x-1" />
            <span className="font-medium">{rawNeighbours.previous.name}</span>
          </Link>
        ) : (
          <div />
        )}
        ...
        {rawNeighbours?.next ? (
          <Link
            href={rawNeighbours.next.url}
            className="group flex items-center gap-2 text-muted-foreground transition-colors hover:text-foreground"
          >
            <span className="font-medium">{rawNeighbours.next.name}</span>
      ...
      >
        <div className="relative mt-6 sm:mt-0">
          <div className="absolute top-1 right-0 flex items-center gap-2">
            <PageNavigationArrows previous={rawNeighbours?.previous} next={rawNeighbours?.next} />
          </div>
